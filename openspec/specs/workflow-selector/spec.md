# workflow-selector Specification

## Purpose

新建任务弹窗中的工作流选择器（`WorkflowSelectorRow`）：约束其下拉列表在多工作流场景下的高度与纵向滚动，确保桌面端与移动端都能完整浏览并选中全部工作流。
## Requirements
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| 仅加 `max-h` + `overflow-y-auto` 后移动端仍无法触摸滚动 | Popover 默认 `portal=true` 渲染到 `document.body`，而 modal `TaskCreateDialog` 打开期间 body 被设 `pointer-events: none` + scroll lock，触摸/滚轮事件被拦截 | 复用 `useTaskCreateDialogPopoverContainer()` 将 PopoverContent 渲染进 dialog 容器，并加 `pointer-events-auto` + `onWheel stopPropagation`（对齐 `components/combobox.tsx`） |
