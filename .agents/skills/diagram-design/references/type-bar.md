...
Two of these rules live in a formula rather than a drawing, so they belong in a project-specific generator check: resolve the domain over every sign case, assert finite coordinates and 3:1 marks, and exercise both polarities — including all-zero and zero-touching data, and the sub-3:1 treatments this replaced. Keep the implementation and this reference synchronized. A shipped dumbbell example would additionally owe a check of drawn positions against the values printed at them.
