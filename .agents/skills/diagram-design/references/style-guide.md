...
This working copy is onboarded to Kandev's visual system. It keeps the docs
paper and the product's near-black canvas as paired light/dark variants, with
indigo as the focal color and cyan/teal reserved for links and multi-series
data. Change these values (or run [`onboarding.md`](onboarding.md)) and every
new diagram inherits the skin without touching type-specific layout logic.

To generate your own from a website URL, see [`onboarding.md`](onboarding.md).

> **Kandev onboarding receipt:** The working skin was sampled from
> [kandev.ai](https://kandev.ai/) and its [public docs](https://kandev.ai/docs/)
> on 2026-09-02. The product surface supplied the dark canvas and indigo
> accent; the docs surface supplied the light paper, ink, and muted rules.
...
| Role | Purpose | Kandev (light) | Kandev (dark) |
|---|---|---|---|
| `paper` | Page background, default node fill | `#f5f5f5` (docs canvas) | `#0d0d10` (product canvas) |
| `paper-2` | Diagram container bg, secondary fill | `#ffffff` | `#111114` |
| `ink` | Primary text, primary stroke | `#0a0a0a` | `#ffffff` |
| `muted` | Secondary text, default arrow stroke | `#737373` | `#a1a1a1` |
| `soft` | Sublabels, boundary labels | `#a1a1a1` | `#737373` |
| `rule` | Hairline borders | `rgba(10,10,10,0.12)` | `rgba(255,255,255,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#d4d4d4` | `#404040` |
| `accent` | Focal / 1–2 max per diagram | `#4f46e5` (indigo primary) | `#6468f0` (indigo primary) |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(79,70,229,0.10)` | `rgba(100,104,240,0.16)` |
| `link` | HTTP/API calls, external arrows | `#0369a1` (sky link) | `#53eafd` (cyan link) |

> **Brand palette source:** light values follow the Kandev docs canvas (`#f5f5f5`, `#0a0a0a`, `#737373`, and low-opacity dark rules); dark values follow the product canvas (`#0d0d10`, `#ffffff`, `#6468f0`, and low-opacity white rules). Cyan/teal are kept for links and the site's activity/data register, not general node emphasis.

> **Note:** The pre-baked `example-*.html` files in `assets/` are historical
> gallery references built under an earlier skin. They remain on the documented
> lint baseline; new diagrams and the public-doc sources in this repository use
> the tokens above.

### Inversion rule (light → dark)

Any `rgba(10,10,10, X)` in light becomes `rgba(255,255,255, X)` in dark. Same opacities, RGB flipped. The indigo accent shifts from `#4f46e5` to the brighter dark-surface `#6468f0`.
...
|---|---|---|---|
| `series-1` | `#0369a1` (sky) | `#53eafd` | Non-focal series |
| `series-2` | `#007a6e` (teal) | `#46ecd5` | Non-focal series |
| `series-3` | `#087443` (emerald) | `#5ee9b5` | Non-focal series |
| `series-4` | `#4f46e5` (indigo) | `#a8b6fb` | Non-focal series |
| `series-5` | `#b75000` (amber) | `#ffd236` | Non-focal series |
...
|---|---|---|---|---|
| `title` | Instrument Serif | 1.75rem | 400 | Page H1; retained as the editorial counterpoint to Kandev's sans UI |
| `node-name` | Figtree (sans) | 12px | 600 | Human-readable labels |
...
```html
<link href="https://fonts.googleapis.com/css2?family=Figtree:wght@400;500;600;700&family=Instrument+Serif:ital@0;1&family=Geist:wght@400;500;600&family=Geist+Mono:wght@400;500;600&family=Noto+Sans+KR:wght@400;500;600&family=Noto+Serif+KR:wght@400&display=swap" rel="stylesheet">
```
