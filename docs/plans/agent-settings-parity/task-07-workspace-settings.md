---
id: "07-workspace-settings"
title: "Expose workspace resource settings"
status: done
wave: 7
depends_on:
  - "06-workflow-settings"
plan: "plan.md"
requirements:
...
acceptance_criteria:
  - AC-PLATFORM-AGENT-SETTINGS-PARITY-006.3
...
# Task 07: Expose workspace resource settings
...
- Adopt workspaces, repositories, repository scripts, and repository sets.
- Preserve wire names through shared tagged contracts rather than generating JSON from untagged controller DTOs.
- Validate default profile/executor references, repository membership, branch rules, and copy-file containment.
- Use existing event-publishing services and atomic repository-set updates. Do not call stores directly from MCP.
- Keep ownership reassignment and workspace destruction outside generic settings writes. Reconcile workspace and repository forms.
...
```bash
rtk proxy go -C apps/backend test ./internal/task/service ./internal/task/handlers ./internal/task/catalog ./internal/worktree/copyfiles ./internal/mcp/server ./internal/mcp/handlers
rtk proxy go -C apps/backend run ./cmd/settings-catalog --check
rtk pnpm --dir apps/web exec vitest run lib/settings-discovery/workspace-settings-contract.test.ts components/settings/workspace-settings-reconciliation.test.tsx
rtk pnpm --dir apps/web run typecheck
rtk pnpm --dir apps/web run i18n:check
rtk pnpm --dir apps/web e2e:run --project chromium tests/settings/workspace-settings-mcp.spec.ts
rtk pnpm --dir apps/web e2e:run --project mobile-chrome tests/settings/mobile-workspace-settings-mcp.spec.ts
rtk node scripts/validate-public-docs.mjs
rtk git diff --check
```

Add `TestWorkspaceSettingsParity` and `TestWorkspaceSettingsAuthority` in
`apps/backend/internal/mcp/server/settings_workspace_settings_test.go`.
...
Change a workspace default and repository copy rule, observe the settings page, and reject foreign repository-set membership.
...
- `apps/backend/internal/task/catalog/ (new)`
- `apps/backend/internal/worktree/copyfiles/`
- `apps/web/app/settings/workspace/`
- `apps/backend/internal/backendapp/services.go` and MCP handler composition.
- `apps/backend/internal/mcp/server/settings_workspace_settings_test.go` (new).
- `apps/web/lib/settings-discovery/workspace-settings-contract.test.ts` (new).
- `apps/web/components/settings/workspace-settings-reconciliation.test.tsx` (new): clean updates, dirty conflicts, own acknowledgements, and late replies.
...
Task 06 completes the preceding slice.
...
- Repository identity changes can affect later worktrees. Saved scripts must not execute during discovery or saving.
...
- Registered workspaces, repositories, repository scripts, and repository sets.
- Preserved workspace/resource pairing, repository-set projection, and lifecycle-only fields.
- Covered workspace resource lookup and cross-scope authorization in the focused backend suite.
