...
title: "Expose notification provider settings"
status: done
wave: 13
depends_on:
  - "12-editor-settings"
...
acceptance_criteria:
  - AC-PLATFORM-AGENT-SETTINGS-PARITY-006.7
...
# Task 13: Expose notification provider settings
...
- Adopt provider names, enabled state, typed provider configuration, and event subscriptions.
- Preserve per-user ownership and validate the complete patch before persisting provider/event changes.
- Preserve credential-bearing fields on omission. New reads redact them and never resolve credentials for descriptions.
- Add an explicit create_notification_provider_kandev adapter if missing. Creation does not send a test notification.
- Reconcile the provider editor and report unavailable provider capabilities without dropping metadata.
...
```bash
rtk proxy go -C apps/backend test ./internal/notifications/controller ./internal/notifications/service ./internal/notifications/catalog ./internal/mcp/server ./internal/mcp/handlers
rtk proxy go -C apps/backend run ./cmd/settings-catalog --check
rtk pnpm --dir apps/web exec vitest run lib/settings-discovery/notification-settings-contract.test.ts components/settings/notification-settings-reconciliation.test.tsx
rtk pnpm --dir apps/web run typecheck
rtk pnpm --dir apps/web run i18n:check
rtk pnpm --dir apps/web e2e:run --project chromium tests/settings/notification-settings-mcp.spec.ts
rtk pnpm --dir apps/web e2e:run --project mobile-chrome tests/settings/mobile-notification-settings-mcp.spec.ts
rtk node scripts/validate-public-docs.mjs
rtk git diff --check
```

Add `TestNotificationSettingsParity` and `TestNotificationSettingsAuthority` in
`apps/backend/internal/mcp/server/settings_notification_settings_test.go`.
...
Change event subscriptions on a disposable provider, verify another user's provider is invisible, and observe reload persistence.
...
- `apps/backend/internal/notifications/dto/dto.go`
- `apps/backend/internal/notifications/controller/controller.go`
- `apps/backend/internal/notifications/service/service.go`
- `apps/backend/internal/notifications/catalog/ (new)`
...
- `apps/backend/internal/mcp/server/settings_notification_settings_test.go` (new).
- `apps/web/lib/settings-discovery/notification-settings-contract.test.ts` (new).
- `apps/web/components/settings/notification-settings-reconciliation.test.tsx` (new): clean updates, dirty conflicts, own acknowledgements, and late replies.
...
Task 12 completes the preceding slice.
...
- Provider configuration can contain secret URLs. Subscription writes must not partially commit an invalid provider patch.
...
- Registered notification provider settings with explicit sensitive-field handling.
- Preserved provider lifecycle and credential-enrollment boundaries.
- Covered notification descriptors, redaction, and adapter behavior in the focused backend suite.
