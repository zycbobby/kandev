---
id: "08-executor-settings"
title: "Expose execution resource settings"
status: done
wave: 8
depends_on:
...
acceptance_criteria:
  - AC-PLATFORM-AGENT-SETTINGS-PARITY-006.4
...
# Task 08: Expose execution resource settings
...
- Adopt editable agent definitions, executors, executor profiles, and environments.
- Preserve executor-type schemas, system-resource restrictions, retained-resource guards, and omitted credential preservation.
- Classify installation, computed capabilities, launcher ownership, and agent permission changes as explicit exceptions.
- Keep supported secret references as references. Redact literal credential paths and reject redaction placeholders as updates.
- Reuse profile save contributors and existing state events. Do not connect, provision, or install an executor during a settings update.
...
- Add coverage bindings and scoped editor tests. Update `docs/public/executors.md` with delivered behavior.
...
```bash
rtk proxy go -C apps/backend test ./internal/task/service ./internal/task/handlers ./internal/task/catalog ./internal/agent/settings/controller ./internal/agent/settings/catalog ./internal/agent/executor ./internal/mcp/server ./internal/mcp/handlers
rtk proxy go -C apps/backend run ./cmd/settings-catalog --check
rtk pnpm --dir apps/web exec vitest run lib/settings-discovery/executor-settings-contract.test.ts components/settings/executor-settings-reconciliation.test.tsx
rtk pnpm --dir apps/web run typecheck
rtk pnpm --dir apps/web run i18n:check
rtk pnpm --dir apps/web e2e:run --project chromium tests/settings/executor-settings-mcp.spec.ts
rtk pnpm --dir apps/web e2e:run --project mobile-chrome tests/settings/mobile-executor-settings-mcp.spec.ts
rtk node scripts/validate-public-docs.mjs
rtk git diff --check
```

Add `TestExecutorSettingsParity` and `TestExecutorSettingsAuthority` in
`apps/backend/internal/mcp/server/settings_executor_settings_test.go`.
...
Change a local executor profile preparation setting, observe the editor, and reject invalid kind-specific settings without launching an executor.
...
- `apps/backend/internal/task/handlers/executor_handlers.go`
...
- `apps/backend/internal/mcp/server/settings_executor_settings_test.go` (new).
- `apps/web/lib/settings-discovery/executor-settings-contract.test.ts` (new).
- `apps/web/components/settings/executor-settings-reconciliation.test.tsx` (new): clean updates, dirty conflicts, own acknowledgements, and late replies.
...
Task 07 completes the preceding slice.
...
- Opaque executor maps contain credentials and kind-specific policy. A permissive map schema cannot replace existing admission checks.
...
- Registered executors, executor profiles, and environments with workspace-scoped targets.
- Preserved computed status, sensitive configuration, and capability boundaries.
- Covered executor domain descriptors and adapter validation in the focused backend suite.
