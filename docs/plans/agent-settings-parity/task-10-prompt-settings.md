---
id: "10-prompt-settings"
title: "Expose saved prompt settings"
status: done
wave: 10
depends_on:
  - "09-task-settings"
...
# Task 10: Expose saved prompt settings
...
- Adopt saved prompt names and content through the current prompt controller.
- Preserve built-in prompt restrictions and current reference expansion semantics.
- Add an explicit create_prompt_kandev adapter if no equivalent tool exists, using a validated settings object.
- Expose content only through authorized reads, never search snippets or resource lookup.
- Reconcile prompt editor drafts and publish or consume the appropriate domain invalidation.
...
```bash
rtk proxy go -C apps/backend test ./internal/prompts/controller ./internal/prompts/service ./internal/prompts/catalog ./internal/mcp/server ./internal/mcp/handlers
rtk proxy go -C apps/backend run ./cmd/settings-catalog --check
rtk pnpm --dir apps/web exec vitest run lib/settings-discovery/prompt-settings-contract.test.ts components/settings/prompt-settings-reconciliation.test.tsx
rtk pnpm --dir apps/web run typecheck
rtk pnpm --dir apps/web run i18n:check
rtk pnpm --dir apps/web e2e:run --project chromium tests/settings/prompt-settings-mcp.spec.ts
rtk pnpm --dir apps/web e2e:run --project mobile-chrome tests/settings/mobile-prompt-settings-mcp.spec.ts
rtk node scripts/validate-public-docs.mjs
rtk git diff --check
```

Add `TestPromptSettingsParity` and `TestPromptSettingsAuthority` in
`apps/backend/internal/mcp/server/settings_prompt_settings_test.go`.
...
Create and update a disposable prompt, observe its editor, reload, and preserve an unsaved competing draft.
...
- `apps/backend/internal/prompts/dto/dto.go`
- `apps/backend/internal/prompts/controller/controller.go`
- `apps/backend/internal/prompts/service/`
- `apps/backend/internal/prompts/catalog/ (new)`
...
- `apps/backend/internal/mcp/server/settings_prompt_settings_test.go` (new).
- `apps/web/lib/settings-discovery/prompt-settings-contract.test.ts` (new).
- `apps/web/components/settings/prompt-settings-reconciliation.test.tsx` (new): clean updates, dirty conflicts, own acknowledgements, and late replies.
...
Task 09 completes the preceding slice.
...
- Prompt bodies are untrusted user data and can contain credentials. Metadata search must not inspect them.
...
- Registered saved prompt settings and routed supported values through the existing domain path.
- Preserved prompt ownership and content validation boundaries.
- Covered prompt descriptors and adapter behavior in the focused backend suite.
