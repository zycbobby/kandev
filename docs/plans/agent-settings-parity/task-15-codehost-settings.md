---
id: "15-codehost-settings"
title: "Expose code-host integration settings"
status: done
wave: 15
depends_on:
  - "14-issue-integration-settings"
plan: "plan.md"
requirements:
...
# Task 15: Expose code-host integration settings
...
- Adopt GitHub, GitLab, and Azure DevOps workspace defaults, repository scope, presets, and watch configuration.
- Keep personal saved-query preferences in user settings and provider workspace settings in their domain.
- Preserve credential generation, provider-host identity, repository scope, and authorized watch references.
- Exclude account enrollment, token operations, remote issue/PR mutations, and immediate watch execution.
- Retain domain cache/event behavior and reconcile each provider's settings and watch editor.
...
```bash
rtk proxy go -C apps/backend test ./internal/github ./internal/gitlab ./internal/azuredevops ./internal/mcp/server ./internal/mcp/handlers
rtk proxy go -C apps/backend run ./cmd/settings-catalog --check
rtk pnpm --dir apps/web exec vitest run lib/settings-discovery/codehost-settings-contract.test.ts components/settings/codehost-settings-reconciliation.test.tsx
rtk pnpm --dir apps/web run typecheck
rtk pnpm --dir apps/web run i18n:check
rtk pnpm --dir apps/web e2e:run --project chromium tests/settings/codehost-settings-mcp.spec.ts
rtk pnpm --dir apps/web e2e:run --project mobile-chrome tests/settings/mobile-codehost-settings-mcp.spec.ts
rtk node scripts/validate-public-docs.mjs
rtk git diff --check
```

Add `TestCodeHostSettingsParity` and `TestCodeHostSettingsAuthority` in
`apps/backend/internal/mcp/server/settings_codehost_settings_test.go`.
...
Update a scope or preset and a watch fixture for each provider, observe the UI, and prove cross-workspace isolation.
...
- `apps/backend/internal/gitlab/service_config.go`
- `apps/backend/internal/gitlab/watch_models.go`
- `apps/backend/internal/azuredevops/workspace_settings.go`
- `apps/backend/internal/azuredevops/watch_service.go`
- `apps/backend/internal/{github,gitlab,azuredevops}/settings_catalog.go (new)`
- `apps/web/app/settings/integrations/`
- `apps/backend/internal/backendapp/services.go` and MCP handler composition.
- `apps/backend/internal/mcp/server/settings_codehost_settings_test.go` (new).
- `apps/web/lib/settings-discovery/codehost-settings-contract.test.ts` (new).
- `apps/web/components/settings/codehost-settings-reconciliation.test.tsx` (new): clean updates, dirty conflicts, own acknowledgements, and late replies.
...
Task 14 completes the preceding slice.
...
- Provider identity and workspace scope are authorization boundaries. A settings update cannot borrow another workspace's credential context.
...
- Registered GitHub, GitLab, Azure DevOps, and code-host watch settings.
- Routed noncredential fields through provider-owned integration services with workspace authorization.
- Preserved connection identity and secret boundaries and covered provider adapters in backend tests.
